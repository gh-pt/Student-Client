...
export default function Input() {
  ...
  const tooltips = {
    ...
    guardianGlobalNo: "Enter one or multiple Guardian Global Numbers (comma-separated).",
  ...
  async function validateInput(event) {
    ...
    const inputs = input.split(",").map((item) => item.trim()).filter(Boolean);
    if (inputs.length === 0) {
